  ...
  constructor(databasePlugin, documentReference) {
  	...
  	this.lastSnapshot = null;
		...
		client.emit('command', {
			...
			args: { ref: DatabaseSerializer.serializeDocumentReference(documentReference), initialValues: true }
		}, (response) => {
			if (response.error) {
				...
				client.on(id, (update) => {
					if (update.error) {
						...
						const result = { newSnapshot, oldSnapshot, changeType: update.changeType };
